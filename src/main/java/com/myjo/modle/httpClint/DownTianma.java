...
@Service
@EnableScheduling
public class DownTianma {

	@Autowired
	private AccessMethod ams;

	@Value("${tianmaQuery}")
	private String tianmaQuery;
	private String time = null;
	private static final Logger LOGGER = LoggerFactory.getLogger(DownTianma.class);
	// 用来保存数据
			...
			for (int i = 0; i < queryJson.length(); i++) {
				...
				for (int m = 0; m < quarters.length; m++) {
					for (int n = 0; n < seasons.length; n++) {
						...
						form1.add(new BasicNameValuePair("quarter", quarters[m] + seasons[n]));
						formEntity = new UrlEncodedFormEntity(form1, "utf-8");
						HttpPost total = new HttpPost("http://www.tianmasport.com/ms/Inventory/groupList.do");
						total.setEntity(formEntity);
						total.setConfig(requestConfig);
						CloseableHttpResponse resps = httpClient.execute(total);
						totalStr = EntityUtils.toString(resps.getEntity(), "utf-8");
						...
						if (totalJson.getInt("total") <= 50000) {
							...
							HttpPost subForm = new HttpPost("http://www.tianmasport.com/ms/Inventory/downGroup.do");
							subForm.setConfig(requestConfig);
							subForm.setEntity(formEntity);
							CloseableHttpResponse res = httpClient.execute(subForm);
							...
							this.downDataTianMa(result, brand_name, "男", "鞋", quarters[m], seasons[n], httpClient);
							continue;
						}
						...
						for (int j = 0; j < sexs.length; j++) {
							for (int k = 0; k < divisions.length; k++) {
								...
								form2.add(new BasicNameValuePair("quarter", quarters[m] + seasons[n]));
								...
								CloseableHttpResponse resp = httpClient.execute(subForms);
								...
								this.downDataTianMa(result, brand_name, sexs[j], divisions[k], quarters[m], seasons[n],
										httpClient);
							}
						}
					}
